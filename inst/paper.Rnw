...
\section{Two approaches of network visualizations}
...
ggplot(data = bloodnet, aes(from_id = from, to_id = to)) +
  geom_net(colour="darkred", layout="circle", label=TRUE, size=12,
           directed=TRUE, vjust=0.5, labelcolour="grey80",
           arrowsize=1.5, linewidth=0.75, arrowgap=0.04) + theme_net()
@

<<blood_ex_ggnet2, echo=TRUE, fig.cap= "Directed network of blood type donations in humans.", fig.width=6, fig.height=6>>=
ggnet2(network(blood$edges[, 1:2]), mode = "circle", size = 12,
       label = TRUE, arrow.size = 7.5, arrow.gap=0.04, vjust = 0.5,
...
ggplot(data = emailnet, aes(from_id = From, to_id = to)) +
  geom_net(aes(colour= CurrentEmploymentType),
           linewidth=0.5, ealpha=0.5, size=4, directed=FALSE) +
...
ggnet2(em.net, size = 4, color = "curr_empl_type", palette = "Set1",
       arrow.size = 0, edge.alpha = 0.5, color.legend = "Employment Type") +
  theme(legend.position="bottom")
@
%
\begin{figure}
...
ggplot(data = emailnet, aes(from_id = From, to_id = to)) +
  geom_net(aes(colour= CurrentEmploymentType), fiteach=TRUE,
           linewidth=0.5, ealpha=0.5, size=2) +
...
for (i in 1:length(em.day)) {
  g[[ i ]] = ggnet2(em.day[[ i ]], size = 2, color = "curr_empl_type",
                    palette = "Set1", arrow.size = 0, edge.alpha = 0.5,
...
ggplot(data = MM2net, aes(from_id = Name1, to_id = Name2)) +
  geom_net(aes(colour=Gender), directed=TRUE, label=TRUE,
           size=2.5, layout='fruchtermanreingold') +
  scale_colour_manual(values=c( "#FF69B4", "#0099ff")) +
...
sid <- which(ftnet$from %in% c("Navy", "NotreDame", "UtahState", "CentralFlorida", "Connecticut"))
ftnet$schools <- ""
ftnet$schools[sid] <- ftnet$from[sid]

ggplot(data=ftnet, aes(from_id=from, to_id=to)) +
  geom_net(aes(colour=value, linewidth=(2-in.conf)/3,  label=schools),
           size=4, ecolour="grey30", vjust=-0.5,
           alpha=0.3, layout='fruchtermanreingold') +
...
ggplot(data = lesmisnet, aes(from_id = from, to_id = to, linewidth = value/5)) +
  geom_net(alpha = 7/10, colour = "red", size = 2.5,
...
ggplot(aes(from_id=Start.station, to_id=End.station), data=tripnet) +
  ...
  scale_colour_manual(values=c("grey40", "darkorange"))
...
yeast.samp <- list(edges = sample.edges, vertices = data.frame(label = unique(c(sample.edges$from,sample.edges$to))))

proteinnet <- merge(yeast.samp$vertices, yeast.samp$edges, by.x="label", by.y="from", all=TRUE)
@
<<yeast_ex, echo=TRUE, fig.cap="Protein in protein interation network in yeast.", cache=FALSE>>=
ggplot(data = proteinnet, aes(from_id = label, to_id = to)) +
  geom_net(alpha=0.25, ealpha = .1, colour = 'magenta', layout = 'random',
