...
Fortunately, R provides function that allow for an easy way of producing the required result: both \code{merge} and \code{join} can be used. In \code{merge} the paramter all needs to be set to TRUE, in \code{join}, the parameter setting has to be \code{type=`full'}, and \code{all=TRUE}.
...
ggplot(data = TEnet, aes(from_id = parent, to_id = child)) +
  geom_net(aes(fontsize=degree), directed = TRUE, label = TRUE, vjust=-.5,
           size=3, ecolour="grey70", curvature=0.25) +
