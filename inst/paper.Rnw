...
\par The three necessary elements of any network visualization are the vertices, the edges, and the layout.   \\
...
\hh{XXXX not sure where this next paragraph should go - but we need a more detailed discussion of ggnet anyways.}
...
\hh{this section needs a better name, but we need it, because here is where your work starts. it should not be part of the introduction.}
...
ggplot(data = TEnet, aes(from_id = parent, to_id = child)) +
  geom_net(directed = TRUE, label = TRUE, vjust=-.5) +
