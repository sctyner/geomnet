...
#yeast <- list(edges = protein, vertices = data.frame(label = min(yeast):max(yeast)))
yeast <- protein
@
...
<<email_ex_facet, fig.height=8, fig.width=8, fig.cap = 'All emails within a company over a two week period facetted by day of the month.',echo=TRUE,eval=FALSE>>=
ggplot(data = subset(email.net, nrecipients < 54), aes(from_id = From, to_id = to)) +
