...
\texttt{"fruchermanreingold"} & uses a variant of Fruchterman and Reingold's force-directed placement algorithm \\
...
\end{tabular}
\caption{All possible layouts to pass to \texttt{geom\_net}.  All descriptions are from \citet{sna}.  Note that the layouts marked with an asterisk (*) are the only layouts that do not take layout parameters.  All others use the default layout parameters in \texttt{sna} for now.}
\label{table:layouts}
\end{table}

All layouts that \texttt{geom\_net} is currently capable of graphing are listed in table \ref{table:layouts}. There is not currently functionality to add layout parameters to the layout function within \texttt{geom\_net}, though this will be included in the final version.  Therefore, all functions in table \ref{table:layouts} that are not marked with asterisks are using their respective default parameters according to the \texttt{sna} documentation for \texttt{gplot.layout}.  
...
<<email_ex, fig.cap = 'All emails within a company over a two week period.',echo=TRUE>>=
...
<<email_ex_facet, fig.height=8, fig.width=8, fig.cap = 'All emails within a company over a two week period facetted by day of the month.',echo=TRUE>>=
ggplot(data = subset(email.net, nrecipients < 54), aes(from_id = From, to_id = to)) + 
...
\hh{XXX have a look at the ggplot2 help online and include something along these lines for both stat\_net and geom\_net}
