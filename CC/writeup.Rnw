...
\section{More Thoughts (26 Oct 14)}
I was thinking some more about the data structure necessary to create a network, and I think it might be possible to just use one data frame.  If we have a data frame (called \texttt{df} below) with ``from" and ``to" columns as the ``x" and ``y" columns, respectively, we could get the vertex names from that by just using an internal command like: 

<<r1, eval=FALSE>>=
vnames <- unique(c(unique(df$x), unique(df$y)))
@

This could simply be the default for the \texttt{vlabel} aesthetic. Then, the \texttt{vlabel} aesthetic could really just be a logical one, which gets the label of \texttt{vnames} as above if \texttt{vlabel = TRUE} and gets nothing if \texttt{vlabel = FALSE}.  To handle the rest of the aesthetics with just one dataframe,  the user would have to use \texttt{merge} or something of the like to join the vertex characteristics to the edge data frame and characteristics.  An example of this with the blood data set is below: 
...
blood.vertices <- unique(blood[,c(1,3)])
names(blood.vertices) <- c("type", "group")
blood.edges <- blood[,1:2]
@
<<r3, tidy=TRUE>>=
blood.vertices
head(blood.edges)

names(blood.vertices) <- c('from', 'group')
blood.df <- merge(blood.edges, blood.vertices, by = 'from')
head(blood.df)
@
After this is done, the \texttt{vcolor} aesthetic could simply be set to \texttt{vcolor = group} and there wouldn't be any need for a separate data frame to be required to pass to \texttt{geom\_ net}. Furthermore, all of the edge aesthesics could simply be set to the variables that are columns in the \texttt{df}, which were determined prior to merging the vertex information with the edge information.  For example, suppose we wanted to color the edges of the blood donation network by whether or not the blood in the ``from" vertex is going to the same or different blood group. i.e. the edge from O- to O+ is the same color as the edge from A- to A+, etc. but the edge from O- to AB+ is a different color. Then the data would have looked like:

<<r4, tidy=TRUE>>=
blood.edges$group_to <- c(rep('same',3),rep('diff',2),rep('same',2),'diff','same',rep('diff',2),rep('same',2),'diff','same',rep('diff',6),rep('same',2),rep('diff',3),'same')
head(blood.edges)
blood.df <- merge(blood.edges, blood.vertices, by = 'from')
head(blood.df)
@

Then, the \texttt{ecolor} aesthestic would just be set to \texttt{ecolor = group\_to}.
\end{document}