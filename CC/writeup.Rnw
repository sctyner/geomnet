...
opts_chunk$set(tidy = TRUE, echo=FALSE, message = FALSE, fig.height = 4, fig.width = 4, fig.align = 'center')
@
<<readingdata>>=
 library(knitr)
...
% <<lesmis_ex>>=
% ggplot(data = lesmis$edges, aes(from_id = from, to_id = to)) + geom_net(vertices = lesmis$vertices, aes(esize = value), layout = 'fruchtermanreingold') + expand_limits(x = c(-25,25), y = c(-25,25)) + theme_net
% @
}
