...
library(knitr)
opts_chunk$set(tidy = FALSE, echo=FALSE, cache=TRUE, eval = TRUE, message = FALSE, warning = FALSE,  fig.height = 8, fig.width = 8, fig.align = 'center',fig.show='hide')
@
...
ggplot(data = football$edges, aes(from_id = from, to_id = to)) +
  geom_net(vertices = football$vertices, ealpha = 0.2, vsize = 5, vlabel = TRUE, aes(vcolour=c("#A6CEE3","#1F78B4","#B2DF8A","#33A02C","#FB9A99","#E31A1C","#FDBF6F","#FF7F00","#CAB2D6","#6A3D9A","#FFFF99","#B15928")[as.numeric(factor(value))])) +
  expand_limits(x = c(0,1), y = c(0,1)) +
  theme_net
@
\sct{
