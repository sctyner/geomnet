...
\end{abstract}
\hh{XXX I've set tidy to FALSE; for some reason it's totally messing up the line width.}
<<chunk_setup, echo=FALSE>>=
library(knitr)
opts_chunk$set(tidy = FALSE, echo=FALSE, eval = FALSE, message = FALSE, warning = FALSE,  fig.height = 8, fig.width = 8, fig.align = 'center')
@
<<readingdata>>=
...
At its core, a network is simply a set of points connected in pairs by a set of lines \citep{newman}.  Here, we refer to the lines as edges and the points as vertices, although these are also called nodes.  These two seemingly simple sets of graphical objects, points and segments, are used to encode a huge variety and quantity of information across many fields of study \hh{XXX this would be a good point to cite a couple of papers that investigate networks}.  Networks vary widely in scope and complexity: the smallest network is simply an edge between two vertices, while one of the most commonly used networks, the world wide web, has billions of vertices (webpages) and billions of edges (hyperlinks) connecting them.  The edges in a network can  be directed or undirected: directed edges represent information travelling from one vertex to another, and switching the direction would change the structure of the network. The world wide web is an example of a directed network because one webpage can link to another, and not necessarily the other way around. Undirected edges, however, are simply connections between vertices.  Coauthorship networks that encode information about academic publications are examples of undirected networks because if two people author a paper together, that creates a connection between them that is bidirectional.
...
This network is shown in figure~\ref{fig.cap:madmen_ex}. \emph{Notes: add names, color for gender}
\hh{XXX if you show the code, you should acknowledge it in the write-up. I think, for the motivating example here, it would be OK to leave out the code ... for the examples later on, the code is much more important.}

<<madmen_ex, dependson='madmen-prep', fig.cap = 'Network of Relationships in Mad Men', echo=TRUE>>=
ggplot(data = madmen$edges, aes(from_id = Name1, to_id = Name2)) +
  ...
  expand_limits(x = c(-7.5,7.5), y = c(-7.5,7.5)) + theme_net
@

\begin{figure}
\centering
\includegraphics{figure/madmen_ex-1.pdf}
\caption{\label{fig.cap:madmen_ex} Graph of the characters in the show Mad Men who are linked by a romatic relationship.}
\end{figure}
...
\hh{XXX How widely used is ggplot2 at the moment? We could include some statistics from \url{http://cran-logs.rstudio.com/} to demonstrate this.}
...
Coloring the vertices or edges in a graph is a quick and easy way to visualize grouping and can help with pattern or cluster detection. The vertices in a network and the edges between them compose the structure of a network, and being able to discover patterns among them visually is a key part of network analysis. Viewing multiple layouts of the same network can also help reveal patterns or clusters that would not be discovered when only viewing one layout or analyzing only an adjacency matrix. The \texttt{ggnet} function, however, requires the graph input to be a \texttt{network} object according to the \texttt{network} package.  Our work builds off of the \texttt{ggnet} function and presents an intuitive \texttt{geom} for network visualization within the \texttt{ggplot2} framework, without the need for objects other than simple data frames.
  ...
  \hh{XXX we need a link between the two data frames: we have from_id and to_id in the edge data frame, but we also need an 'id' variable in the vertex data. We don't officially have that at the moment - in the code there is a soft requirement (new as of Mar 20) that one of the columns in the edge data set has to be called 'label' ... that's a bit stoopid, but I couldn't think of a better name. It would make more sense to call this id or edgeid or something along those lines - let's talk about it, and make it a formal requirement. }
...
ggplot(data = blood$edges, aes(from_id = from, to_id = to)) +
  geom_net(vertices = blood$vertices, layout='circle', vlabel = TRUE) +
  expand_limits(x = c(-2,2), y = c(-2,2)) + theme_net #add vlabel & direction later
@
\sct{
...
ggplot(data = email.net, aes(from_id = From, to_id = to, )) +
  geom_net(vertices = email.names,ealpha = 0.1, vsize = I(2)) +
  expand_limits(x=c(-1.75,1.75), y = c(-1.75,1.75)) +
  theme_net #add direction later
@
...
<<theme_ex, echo=TRUE, fig.cap= 'Inheritance structure for \\texttt{theme} elements in \\texttt{ggplot2}.', eval=FALSE>>=
ggplot(data = theme_elements$edges, aes(from_id = parent, to_id = child)) +
  geom_net(vertices = theme_elements$vertices) +
  expand_limits(x = c(-5,5), y = c(-5,5)) +
  theme_net #add vlabel & direction later
@
...
ggplot(data = football$edges, aes(from_id = from, to_id = to)) +
  geom_net(vertices = football$vertices, ealpha = 0.1) +
...
ggplot(data = lesmis$edges, aes(from_id = from, to_id = to)) +
  geom_net(vertices = lesmis$vertices, aes(esize = value), ealpha = 2/10, layout = 'fruchtermanreingold') +
...
ggplot(data = yeast$edges, aes(from_id = V1, to_id = V2)) +
  ...
  expand_limits(x = c(-25,25), y = c(-25,25)) + theme_net
@
