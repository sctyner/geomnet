...
ggplot(data = yeast.samp$edges, aes(from_id = V1, to_id = V2)) +
  geom_net(vertices = yeast.samp$vertices, ealpha = .25, valpha = .5, vcolour = 'magenta',
...
stat_net(mapping = NULL, data = NULL, vertices = NULL, geom = "net", position = "identity", na.rm = FALSE, ...)
#aes(colour = "black", size =3, layout="kamadakawai",layout.par = NA)
@
  The usage of the \texttt{stat\_net} arguments is as follows:
  \sct{XXX adding detail tomorrow}
  \begin{itemize}
  \item mapping
  \item data
  \item vertices
...
@
  ...
  \item stat
  \end{itemize}
  \texttt{stat\_net} also accepts the following aesthetic values, and the ones marked with an asterisk are required:
  \begin{itemize}
  \item from\_id*
  \item to\_id*
  \item ecolour
...
\section{Future Work}
\hh{XXX  just be a bit more specific here: we (a) want to make the code a part of the ggplot2 world - either as a part of ggplot2 itself or within GGally. (b) we want to publish this paper in the R Journal.
Things that we still need to do are discussed in the paper, they are: .... }
As we discussed throughout the paper, there is still some work to be done  before we submit it for publication in \emph{The R Journal}.  We also hope to make it an official part of \texttt{ggplot2}.  We believe that this is an important contribution to \texttt{ggplot2} and to the R user community because it fills in gaps in the network plotting capabilities of R. \sct{XXX That's all I got. Help?}
