...
ggplot(data = email.net, aes(from_id = From, to_id = to)) +
  geom_net(vertices = email.names,ealpha = 0.1, vsize = I(2), directed = TRUE) +
...
ggplot(data = lesmis$edges, aes(from_id = from, to_id = to, esize = value/mean(value))) + geom_net(vertices = lesmis$vertices, ealpha = 7/10, layout = 'fruchtermanreingold', vlabel=TRUE) + expand_limits(x = c(0,1.1), y = c(0,1)) + theme_net
@
\begin{figure}
\centering
