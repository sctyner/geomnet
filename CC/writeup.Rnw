...
@
<<readingdata, message=FALSE, echo=FALSE>>=
 library(knitr)
 library(ggplot2)
...
In this section, we demonstrate the capabilities of \texttt{geom\_net} in a series of examples. 
  ...
  In this network, there are eight nodes and 27 edges.  The nodes represent the eight different blood types possible in humans
%<<blood_ex,highlight=TRUE>>=
...
% # @
  ...
  \emph{Note: would be best with labels and direction}
% # <<theme_ex>>=
% # #ggplot(data = g$edges, aes(from_id = child, to_id = parent)) + geom_net(vertices = g$vertices) + expand_limits(x = c(-5,5), y = c(-5,5)) + theme_net
% # @
