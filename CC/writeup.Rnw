...
opts_chunk$set(tidy = TRUE, echo=FALSE, eval = TRUE, message = FALSE, fig.height = 4, fig.width = 4, fig.align = 'center')
@
<<readingdata>>=
 library(knitr)
...
ggplot(data = madmen$edges, aes(from_id = Name1, to_id = Name2)) + geom_net(vertices = madmen$vertices) + expand_limits(x = c(-10,10), y = c(-10,10)) + theme_net
@
\sct{
...
ggplot(data = blood$edges, aes(from_id = from, to_id = to)) + geom_net(vertices = blood$vertices, layout='circle') + expand_limits(x = c(-2,2), y = c(-2,2)) + theme_net #add vlabel & direction later
@
\sct{
