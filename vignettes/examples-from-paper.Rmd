...
MMnet <- fortify(madmen$edges, madmen$vertices)
# create plot
set.seed(10052016)
...
emailnet <- fortify(subset(email$edges, nrecipients < 54), email$nodes, group = "day")
...
TEnet <- fortify(theme_elements$edges[,c("parent", "child")], theme_elements$vertices)
TEnet <- TEnet %>%
  group_by(parent) %>%
